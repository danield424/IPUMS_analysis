...
raw_data <- read_csv("/Users/dudaniel/Downloads/304/IPUMS_data/usa_00001.csv")
...
knitr::kable(
  state_count,
  caption = "Respondents In Each State with Doctoral Degree",
  col.names = c("State Code (STATEICP)", "Number of Respondents"),
  format = "latex",
  booktabs = TRUE,
  linesep = ""
) %>%
...
If, for example, there are 391,171 respondents in California (state 71 according the codebook) across all levels of education, we can use the ratio estimator to estimate the total number of respondents in each state as follows:
...
```{r, echo=FALSE}
state_count2 <- state_count %>% mutate(estimated_total_count = round(count*ratio))
state_count2 <- state_count2 %>% select(STATEICP, estimated_total_count, total_count)
knitr::kable(
  state_count2,
  caption = "Estimated Respondents vs Total Respondents in Each State",
  col.names = c("State Code (STATEICP)", "Actual # of Respondents", "Estimated # of Respondents"),
  format = "latex",
  booktabs = TRUE,
  linesep = ""
) %>%
...
\newpage
Any errors in the estimates are likely because of the following data limitations, discussed below:
